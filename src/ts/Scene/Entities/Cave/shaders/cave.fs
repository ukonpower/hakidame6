...
vec2 D( vec3 p ) {
	...
	float n = fbm(p * 1.0 + fbm3(p * 1.0 ) * 10.0) * 0.5 + 0.5;
	p.z -= n * 0.1;

	vec2 d = vec2( sdBox( p, vec3( 10.0, 10.0, 0.1 ) ), 1.0 );
...
void main( void ) {
	...
	vec3 rayPos = vec3( vUv.xy * 2.0, 1.0 );
	vec3 rayDir = normalize( vec3( 0.0, 0.0, -1.0 ) + vec3(vPos - cameraPosition) );
	vec2 dist = vec2( 0.0 );
	bool hit = false;
	...
	outPos = ( modelMatrix * vec4( vPos, 1.0 ) ).xyz;
